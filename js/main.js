...
var title = "";
// Saves the cast member keyword to search by cast
var cast = "";
// Saves the genre keyword to search by genre
var genre = "";
...
function getInputText() {
  ...
  toPrint = buildHTMLresult(result, "ALL");
  toPrintMovies = buildHTMLresult(resultMovie, "MOVIE");
  toPrintTV = buildHTMLresult(resultTV, "TV");
...
function printFilms() {
  ...
  if (result.length === 0) {
    if (typeof inputText !== 'undefined') {
      textResult = "<p>No results for " + inputText + "</p>";
    } else {
      textResult = "<p>No results</p>";
    }
    document.getElementById("resultTitle").innerHTML = textResult;
    ...
    if (typeof inputText !== 'undefined') {
      textResult = "<p>Top results for " + inputText + "</p>";
    } else {
      textResult = "<p>Top results</p>";
    }
    document.getElementById("resultTitle").innerHTML = textResult;
    ...
    if (resultMovie.length > 0 && resultTV.length <= 0) {
      ...
      document.getElementsByClassName("mCard")[focusIndex].style.left = "88px";
    ...
    if (resultTV.length > 0 && resultMovie <= 0) {
      ...
      document.getElementById("movie").style.visibility = "visible";
      document.getElementById("movie").innerHTML = "<p>TV Shows</p>";
      document.getElementsByClassName("tCard")[focusIndex].style.left = "88px";
      document.getElementById("tvResult").style.visibility = "hidden";
      document.getElementById("tvResult").innerHTML = "";
...
function buildHTMLresult(inputArray, type) {
  ...
  switch (type) {
    ...
    case "MOVIE": startH = "<div class='mCard' onclick='videoClicked(";
      vod = "<div class='vod' hidden><img class='mDots' src='images/whiteDots.png' /></div>";
      break;
    case "TV": startH = "<div class='tCard' aria-label='tv item' onclick='videoClicked(";
      vod = "<div class='vod' hidden><img class='tDots' src='images/whiteDots.png' /></div>";
      break;
  }
...
function filter(vt) {
  ...
  *****/
  ...
  if (cast.length > 0) {
    getMovieResult = [];
    getMovieResult = getMovie(cast);
    if (getMovieResult.length > 0) {
      result = result.concat(getMovieResult);
      console.log("ADDED ALL TITLES THAT INCLUDE: " + cast);
    }
  ...
  if (genre.length > 0) {
    getMovieResult = [];
    getMovieResult = getMovie(genre);
    if (getMovieResult.length > 0) {
      result = result.concat(getMovieResult);
      console.log("ADDED ALL " + genre + " TITLES");
    }
  ...
  **********/
  ...
  var objectIndex, objectArrayIndex, castArrayIndex, genreArrayIndex;
  ...
  switch (filterFlag) {
    ...
    case 2: for (objectIndex = 0; objectIndex < result.length; objectIndex++) {
      for (objectArrayIndex = 0; objectArrayIndex < result[objectIndex].genre.length; objectArrayIndex++) {
        if (result[objectIndex].genre[objectArrayIndex].toLowerCase() === genre.toLowerCase()) {
          tempResult.push(result[objectIndex]);
    ...
    case 3: for (objectIndex = 0; objectIndex < result.length; objectIndex++) {
      for (objectArrayIndex = 0; objectArrayIndex < result[objectIndex].genre.length; objectArrayIndex++) {
        if (result[objectIndex].genre[objectArrayIndex].toLowerCase() === genre.toLowerCase() && result[objectIndex].year === year) {
          tempResult.push(result[objectIndex]);
    ...
    case 4: for (objectIndex = 0; objectIndex < result.length; objectIndex++) {
      for (objectArrayIndex = 0; objectArrayIndex < result[objectIndex].cast.length; objectArrayIndex++) {
        if (result[objectIndex].cast[objectArrayIndex].toLowerCase().includes(cast.toLowerCase())) {
          tempResult.push(result[objectIndex]);
    ...
    case 5: for (objectIndex = 0; objectIndex < result.length; objectIndex++) {
      for (objectArrayIndex = 0; objectArrayIndex < result[objectIndex].cast.length; objectArrayIndex++) {
        if (result[objectIndex].cast[objectArrayIndex].toLowerCase().includes(cast.toLowerCase()) && result[objectIndex].year === year) {
          tempResult.push(result[objectIndex]);
    ...
    case 6: for (objectIndex = 0; objectIndex < result.length; objectIndex++) {
      for (castArrayIndex = 0; castArrayIndex < result[objectIndex].cast.length; castArrayIndex++) {
        for (genreArrayIndex = 0; genreArrayIndex < result[objectIndex].genre.length; genreArrayIndex++) {
          if (result[objectIndex].cast[castArrayIndex].toLowerCase().includes(cast.toLowerCase()) && result[objectIndex].genre[genreArrayIndex].toLowerCase() === genre.toLowerCase()) {
            tempResult.push(result[objectIndex]);
    ...
    case 7: for (objectIndex = 0; objectIndex < result.length; objectIndex++) {
      for (castArrayIndex = 0; castArrayIndex < result[objectIndex].cast.length; castArrayIndex++) {
        for (genreArrayIndex = 0; genreArrayIndex < result[objectIndex].genre.length; genreArrayIndex++) {
          if (result[objectIndex].cast[castArrayIndex].toLowerCase().includes(cast.toLowerCase()) && result[objectIndex].genre[genreArrayIndex].toLowerCase() === genre.toLowerCase() && result[objectIndex].year === year) {
            tempResult.push(result[objectIndex]);
...
// Load content on the main page
function preloaded() {
  result = getNoDummyData();
  resultMovie = [];
  resultTV = [];
  filterByType();
  toPrint = buildHTMLresult(result, "ALL");
...
// Filter data. Exclude dummy data
function getNoDummyData() {
  var results = [];
  // Filter by cast
  films.forEach(function (arrayItem) {
    var castArray = arrayItem.cast;
    for (var i = 0; i < castArray.length; i++) {
      if (!castArray[i].toLowerCase().includes("n/a")) {
        results.push(arrayItem);
        break;
      }
    }
    ...
    switch (rowIndex) {
      case 0:
        ...
        if (resultMovie.length > 0) {
          document.getElementsByClassName("dots")[movieIndex].src = "images/whiteDots.png";
          document.getElementsByClassName("card")[movieIndex].blur();
        }
        if (resultMovie.length > 0) {
...
function newNavRight() {
  ...
  switch (rowIndex) {
    ...
    case 3: if (tvIndex >= resultTV.length - 1) { pendingMove = 0; } break;
  }
  if (pendingMove <= 0) {
    clearInterval(navigationInterval);
    navigationInterval = false;
...
function newNavLeft() {
  ...
  switch (rowIndex) {
    ...
    case 3: if (tvIndex <= 0) { pendingMove = 0; } break;
  }
  if (pendingMove <= 0) {
    clearInterval(navigationInterval);
    navigationInterval = false;
...
function navLeft() {
  switch (rowIndex) {
    case 0: break;
    case 1: // Validate the number of objects in result array and animate accordingly
      if (result.length > 0) {
        if (buttonEnabled) {
          focusIndex -= 1;
          if (focusIndex < 0) {
...
function nextPageShift() {
  ...
  if (rowIndex === 1) {
    ...
    document.getElementsByClassName("mCard")[movieIndex].style.left = "88px";
    newValue = -320;
    ...
    document.getElementsByClassName("tCard")[tvIndex].style.left = "88px";
    newValue = -320;
...
function shiftRight() {
  ...
  if (rowIndex === 1) {
    ...
    if (movieIndex === 0 && document.getElementsByClassName("mCard")[movieIndex].style.left === "88px") {
      return;
    }
    if (rect.left < 500 && movieIndex < (resultMovie.length - 2)) {
      ...
      if (movieIndex > 0) {
        buttonEnabled = false;
        leftPosition = 88 + 344;
        document.getElementsByClassName("mCard")[movieIndex - 1].style.left = "88px";
    ...
    if (tvIndex <= 0 && document.getElementsByClassName("tCard")[tvIndex].style.left === "88px") {
      return;
    }
    if (rect.left < 500 && tvIndex < (resultTV.length - 2)) {
      ...
      if (tvIndex > 0) {
        buttonEnabled = false;
        leftPosition = 88 + 344;
        document.getElementsByClassName("tCard")[tvIndex - 1].style.left = "88px";
...
function shiftLeft() {
  var rect, card, mCard, tCard, i;
  if (rowIndex === 1) {
    rect = document.getElementsByClassName("card")[focusIndex].getBoundingClientRect();
    if (focusIndex >= 3 && focusIndex < result.length && rect.left > 688) {
      for (i = focusIndex; i >= 0; i--) {
        card = document.getElementsByClassName("card")[i];
        if (i !== 0) {
          card.style.left = document.getElementsByClassName("card")[i - 1].getBoundingClientRect().x + 'px';
        } else {
          card.style.left = card.getBoundingClientRect().x - 344 + 'px';
        }
      }
      ...
      for (i = (focusIndex + 1); i < result.length; i++) {
        card = document.getElementsByClassName("card")[i];
        if (i !== (focusIndex + 1)) {
          card.style.left = parseInt(document.getElementsByClassName("card")[i - 1].style.left, 10) + 344 + 'px';
        } else {
          card.style.left = parseInt(document.getElementsByClassName("card")[focusIndex].style.left, 10) + 344 + 'px';
        }
      }
      buttonEnabled = false;
      setTimer = setTimeout(function () { buttonEnabled = true; }, 400);
    ...
    rect = document.getElementsByClassName("mCard")[movieIndex].getBoundingClientRect();
    if (movieIndex >= 3 && movieIndex < resultMovie.length && rect.left > 688) {
      for (i = movieIndex; i >= 0; i--) {
        mCard = document.getElementsByClassName("mCard")[i];
        if (i !== 0) {
          mCard.style.left = document.getElementsByClassName("mCard")[i - 1].getBoundingClientRect().x + 'px';
        } else {
          mCard.style.left = mCard.getBoundingClientRect().x - 344 + 'px';
        }
      }
      // Change position to (movieIndex+`) -> moviesLength position
      for (i = (movieIndex + 1); i < resultMovie.length; i++) {
        mCard = document.getElementsByClassName("mCard")[i];
        if (i !== (movieIndex + 1)) {
          mCard.style.left = parseInt(document.getElementsByClassName("mCard")[i - 1].style.left, 10) + 344 + 'px';
        } else {
          mCard.style.left = parseInt(document.getElementsByClassName("mCard")[movieIndex].style.left, 10) + 344 + 'px';
        }
      }
      buttonEnabled = false;
      setTimer = setTimeout(function () { buttonEnabled = true; }, 400);
    ...
    rect = document.getElementsByClassName("tCard")[tvIndex].getBoundingClientRect();
    if (tvIndex >= 3 && tvIndex < resultTV.length && rect.left > 688) {
      for (i = tvIndex; i >= 0; i--) {
        tCard = document.getElementsByClassName("tCard")[i];
        if (i !== 0) {
          tCard.style.left = document.getElementsByClassName("tCard")[i - 1].getBoundingClientRect().x + 'px';
        } else {
          tCard.style.left = tCard.getBoundingClientRect().x - 344 + 'px';
        }
      }
      // Change position to (movieIndex+`) -> moviesLength position
      for (i = (tvIndex + 1); i < resultTV.length; i++) {
        tCard = document.getElementsByClassName("tCard")[i];
        if (i !== (tvIndex + 1)) {
          tCard.style.left = parseInt(document.getElementsByClassName("tCard")[i - 1].style.left, 10) + 344 + 'px';
        } else {
          tCard.style.left = parseInt(document.getElementsByClassName("tCard")[tvIndex].style.left, 10) + 344 + 'px';
        }
      }
      buttonEnabled = false;
      setTimer = setTimeout(function () { buttonEnabled = true; }, 400);
...
function previousPageShift() {
  ...
  if (rowIndex === 1) {
    ...
    if (movieIndex < 4) {
      document.getElementsByClassName("mCard")[0].style.left = "88px";
      leftPosition = 88;
      ...
      document.getElementsByClassName("mCard")[movieIndex - 3].style.left = "88px";
      leftPosition = 88;
    ...
    if (tvIndex < 4) {
      document.getElementsByClassName("tCard")[0].style.left = "88px";
      leftPosition = 88;
      ...
      document.getElementsByClassName("tCard")[tvIndex - 3].style.left = "88px";
      leftPosition = 88;
