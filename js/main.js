	...
	try{
		webapis.voiceinteraction.setCallback(
		        ...
		        try {
		        	...
		        	for(var i=0; i<result.length; i++){
		        		...
		        		item = {
				                ...
				                "alias": [result[i].rating, result[i].type],
				                "bFocused": bFocused
				            };*/
			...
			/*onrequestcontentcontext : function(){
		        ...
		        var resultArray = [];
		        resultArray.push(webapis.voiceinteraction.buildVoiceInteractionContentContextItem(0,0,"The Avengers", ["Marvel Cinematic"], true));
		        return webapis.voiceinteraction.buildVoiceInteractionContentContextResponse(resultArray);
		    },
			ontitleselection: function(title){
				console.log("ontitleselection : " + title);
				if(result.length > 0){
					for(var i=0; i<result.length; i++){
						if(result[i].title.toLowerCase().includes(title.toLowerCase())){
							console.log(result[i].title + " WAS SELECTED");
							focusIndex = i;
							itemSelected();
							break;
						}
					...
					document.getElementById("desc").innerHTML = "<p>No titles available.</p>";
					launch_toast();
				}
				return true;
			},*/
			...
			ontestdownloadable: function (data) {
            	console.log("ontestdownloadable : " + data);
            	document.getElementById("desc").innerHTML = "<p>" + data + "</p>";
				launch_toast();
            	return true;
            }
			onsharetodevice: function(param){
				console.log("ON SHARE TO DEVICE: " + param);
				document.getElementById("desc").innerHTML = "<p><strong>ACTION:</strong> onsharetodevice<br><strong>PARAMETERS:</strong> " + param + "</p>";
				launch_toast();
				return true;
			},*/
			onskipad: function(){
...
var init = function () {
    ...
    console.log('Version 1.12 | init() called');
    registerKeys();
